...
class AdminOutlawTest < AppTest
  ...
  def insert_outlaw_card(description: 'test', redeemed: false)
    ...
    if redeemed
      qr_id = DB[:qr_tokens].insert(token: SecureRandom.hex(2), minutes: 10, created_at: Time.now.utc.iso8601)
      scan_id = DB[:scan_log].insert(qr_token_id: qr_id, scanned_at: Time.now.utc.iso8601)
  ...
  def post_outlaw(params = {})
    ...
    post '/admin/outlaw', defaults.merge(params), auth_header
  end

  # ─── GET /admin/outlaw — auth ─────────────────────────────────────────────
  ...
  def test_get_returns_200_with_auth
    get '/admin/outlaw', {}, auth_header
    assert_equal 200, last_response.status
  end

  # ─── GET /admin/outlaw — content ─────────────────────────────────────────
  ...
  def test_get_shows_success_notice_when_param_present
    get '/admin/outlaw', { success: '1' }, auth_header
  ...
  # ─── POST /admin/outlaw — auth ────────────────────────────────────────────

  def test_post_requires_basic_auth
    post '/admin/outlaw', { secure_token: ENV['SECURE_TOKEN'] }
    assert_equal 401, last_response.status
  end

  # ─── POST /admin/outlaw — secure token validation ─────────────────────────
  ...
  def test_post_redirects_to_outlaw_success_page
    post_outlaw
    assert_equal 302, last_response.status
    assert_includes last_response.headers['Location'], '/admin/outlaw?success=1'
  end
